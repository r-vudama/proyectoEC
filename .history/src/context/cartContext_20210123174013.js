...
const CarritoProvider = ({children}) => {
    ...
    const [stock, setCantidad] = useState(0);
        ...
        const confirmarCompra = (e) => {
          ...
          OrderCollection.add(datosCompra).then((resultado)=>{
              ...
              carrito.forEach(item=>{
                  batch.update(Itemscollection.doc(item.id), {stock: calcularStock(item.stock, item.cantidad)})
              })
    ...
    const calcularStock = (stock, cantidad) => {
